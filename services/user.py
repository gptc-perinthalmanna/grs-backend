...
async def get_user(username: str) -> Optional[UserInDB]:
    user = await userDB.get_user_from_username_db(username)
    if user:
...
async def update_user_fields(updateuser: UserEdit, user: User) -> Optional[UserInDB]:
    ...
    useroutdb = await userDB.update_user_to_db(User(**updateuser.dict()), userindb)
    return useroutdb
...
async def check_password_and_username(userinput, user: User):
    user_db = await userDB.get_user_from_id(user.key)
